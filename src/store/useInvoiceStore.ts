...
export const useInvoiceStore = create<State & Actions>((set) => ({
    ...
    updateState: (payload: TInvoiceSchema) => set(() => ({
        data: payload
    })),
    ...
    updateSubtotal() {
        return set((state) => {
            const t = state.data.items.map(item => (item.amount * item.quantity) - item.discount)

            return {
                data: {
                    ...state.data,
                    subtotal: t.reduce((accumulator, currentValue) => accumulator + currentValue, 0)
                }
            }
        })
    },
